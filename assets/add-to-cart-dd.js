...
document.addEventListener('DOMContentLoaded', () => {
  ...
  variantRadios.forEach(radio => {
    radio.addEventListener('change', (e) => {
      ...
      console.log('Variant Value:', selectedVariant);
    });
  });

  // Quantity selection handler with pricing logic
  quantityRadios.forEach(radio => {
    radio.addEventListener('change', (e) => {
      selectedQuantity = parseInt(e.target.value);
      
      // Quantity-based pricing logic
      const basePrice = extractPrice(currentPriceEl.textContent);
      let adjustedPrice;
      ...
      // Update price display
      if (currentPriceEl) {
        currentPriceEl.textContent = `$${adjustedPrice.toFixed(2)}`;
      }
    });
  });
  ...
  addToCartBtn.addEventListener('click', async (e) => {
    ...
    // Disable button during cart addition
    ...
    try {
      ...
      console.error('Add to Cart Error:', error);
      showMessage(error.message || 'Failed to add item to cart', false);
    } finally {
      // Reset button
      addToCartBtn.disabled = false;
      addToCartBtn.textContent = 'Add to Cart';
  ...
  console.log('Variant Radios:', variantRadios);
  console.log('Quantity Radios:', quantityRadios);
  console.log('Current Price Element:', currentPriceEl);
  console.log('Compare At Price Element:', compareAtPriceEl);
});