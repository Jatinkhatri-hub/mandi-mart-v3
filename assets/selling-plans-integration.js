class SellingPlansWidget {
  ...
  initializeElements() {
    ...
    // New elements for selling plan toggle
    this.purchaseTypeRadios = document.querySelectorAll('input[name="purchase-type"]');
    this.subscriptionDropdown = document.querySelector('.subscription-plans-dropdown');
    this.sellingPlanDropdownContainer = document.querySelector('.selling-plan-dropdown-container');
  ...
  bindEvents() {
    ...
    // New purchase type (One-time/Subscribe) events
    this.purchaseTypeRadios.forEach(radio => {
      radio.addEventListener('change', (event) => this.handlePurchaseTypeChange(event.target));
    });

    // Selling plan dropdown change event
    if (this.subscriptionDropdown) {
      this.subscriptionDropdown.addEventListener('change', () => this.handleSubscriptionPlanChange());
    }

    // Initial selling plan events
  ...
  handlePurchaseTypeChange(selectedRadio) {
    // Show/hide subscription dropdown based on purchase type
    if (this.sellingPlanDropdownContainer) {
      if (selectedRadio.value === 'subscribe') {
        this.sellingPlanDropdownContainer.style.display = 'block';
        // Automatically select the first subscription option
        if (this.subscriptionDropdown) {
          this.subscriptionDropdown.selectedIndex = 0;
          this.handleSubscriptionPlanChange();
        }
      } else {
        this.sellingPlanDropdownContainer.style.display = 'none';
        // Reset to one-time purchase
        this.handleOneTimePurchase();
      }
    }
  }
  ...
  handleSubscriptionPlanChange() {
    ...
    const selectedPlan = this.subscriptionDropdown.value;
    const currentVariantId = this.getCurrentVariantId();

    // Find the corresponding selling plan radio
    const matchingPlanRadio = document.querySelector(`
      input[name="purchaseOption_${this.getSectionId()}_${currentVariantId}"][data-selling-plan-id="${selectedPlan}"]
    `);

    if (matchingPlanRadio) {
      matchingPlanRadio.checked = true;
      this.handleSellingPlanChange(matchingPlanRadio);
    }
  }
  ...
  handleOneTimePurchase() {
    ...
    const oneTimePurchaseRadio = document.querySelector(`
      input[name="purchaseOption_${this.getSectionId()}_${currentVariantId}"][data-radio-type="one_time_purchase"]
    `);

    if (oneTimePurchaseRadio) {
      oneTimePurchaseRadio.checked = true;
      this.handleSellingPlanChange(oneTimePurchaseRadio);
    }
  }
